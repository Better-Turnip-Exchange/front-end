...
describe(`Select Keyword Test`, () => {
    ...
    keywords.forEach(keyword => {
        ...
        it(`${keyword} should be in rendered keyword list`, () => {
            expect(container.find(`#${keyword}`).length).toEqual(2);
        })
        if (keyword !== 'entry') {
            it(`${keyword} that is initally false should render untoggled`, () => {
                expect(container.find(`button#${keyword}`)
                    .prop('class'))
                    .toEqual('spin keyword-label rounded py-2 px-2 mr-2 shadow-md w-20 bg-gray-100 hover:bg-gray-200 hover:shadow-lg')
            })
        ...
        it(`clicking ${keyword} when true should set ${keyword} to false in state`, () => {
...
describe(`Select Price Test`, () => {
    const container = shallow(<Select />)
    it(`price input should not render blank`, () => {
        expect(container.find(`#price-input`).prop('value')).toBeDefined();
    })
